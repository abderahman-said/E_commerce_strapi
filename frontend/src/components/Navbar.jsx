...
const Navbar = ({ cartCount = 0 }) => {
  ...
  return (
    <>
      ...
      <header className={`sticky top-0 z-40  bg-white/80 backdrop-blur-md border-b transition-all duration-300 ${isScrolled ? 'border-gray-200 shadow-sm' : 'border-transparent'
        }`}>
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-18 lg:h-20 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-8" role="navigation" aria-label="Main navigation">
            <Link
              to="/shop"
              className="text-sm font-semibold uppercase tracking-wide text-gray-900 hover:text-blue-600 transition-colors duration-200 relative group py-2 px-1"
            >
              Shop
            ...
            {categories.map((cat) => (
              <Link
                key={cat.id}
                to={`/shop?category=${cat.slug || cat.name.toLowerCase().replace(/\s+/g, '-')}`}
                className="text-sm font-semibold uppercase tracking-wide text-gray-900 hover:text-blue-600 transition-colors duration-200 relative group py-2 px-1"
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden absolute top-full left-0 w-full bg-white border-t border-gray-200 py-6 px-4 sm:px-6 flex flex-col gap-2 shadow-xl animate-slideInDown"
            ...
            <Link
              to="/shop"
              className="text-base font-semibold uppercase tracking-wide text-gray-900 py-3 px-4 rounded-lg hover:bg-gray-50 transition-all duration-200 flex items-center justify-between group"
